...
/*
@brief: 应用fftshift, 将所有缓存数据进行移位
*/
void SignalPlot::applyFFTShift() {
    //移位所有缓存数据
    auto shiftAll = [this](auto& vec) {
        if(!vec.empty()) shiftVector(vec);
    };

    shiftAll(spectrumCache.magLinear);
    shiftAll(spectrumCache.magDB);
    shiftAll(spectrumCache.psdLinear);
    shiftAll(spectrumCache.psdDB);
}
...
void SignalPlot::plotAmplitudeSpectrum(bool isLogScale, bool isDB) {
    ...
    const auto& displayFreq = fftShiftEnabled ? spectrumCache.shiftFreq : spectrumCache.freq;
    const auto& displayMag = isDB ? spectrumCache.magDB : spectrumCache.magLinear;
    ...
    QVector<double> qFreq = QVector<double>(displayFreq.begin(), displayFreq.end());
    QVector<double> qMag = QVector<double>(displayMag.begin(), displayMag.end());
    const int N = static_cast<int>(displayFreq.size());
...
void SignalPlot::plotPSD(bool isLogScale, bool isDB) {
    ...
    const auto& displayFreq = fftShiftEnabled ? spectrumCache.shiftFreq : spectrumCache.freq;
    const auto& displayPSD = isDB ? spectrumCache.psdDB : spectrumCache.psdLinear;
    ...
    QVector<double> qFreq = QVector<double>(displayFreq.begin(), displayFreq.end());
    QVector<double> qPSD = QVector<double>(displayPSD.begin(), displayPSD.end());
    const int N = static_cast<int>(displayFreq.size());
...
void SignalPlot::showContextMenu(const QPoint &pos) {
    ...
    //频谱类绘图显示切换x轴log/y轴dB, 以及fftShift
    if(currentPlotType == Amplitude || currentPlotType == PSD) {
        ...
        //fftshift
        QAction* fftShiftAction = menu.addAction("fftShift");
        fftShiftAction->setCheckable(true);
        fftShiftAction->setChecked(fftShiftEnabled);
        connect(fftShiftAction, &QAction::toggled, this, &SignalPlot::setFFTShiftEnabled);
...
void SignalPlot::setFFTShiftEnabled(bool checked) {
    if(fftShiftEnabled != checked) {
        fftShiftEnabled = checked;
        applyFFTShift();
        replotCurrentGraph();
    }
    else {
